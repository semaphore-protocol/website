...
Go to the `contracts` folder by running:
...
3. Go to the `apps/contracts` folder and deploy your contract.
    ...
    :::note
    Check the Semaphore contract addresses [here](/docs/deployed-contracts).
    :::
...
:::note
Make sure you have added the environment variables in a `.env` file in the project root folder. The required ones are: the default network, the Infura API Key, the private key, the Semaphore contract address, the Feedback contract address and the group id.
::: 
...
You can start you app locally with:
